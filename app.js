...
for (let i = 0; i <= points; i++) {
  ...
  // Update DOM elements
  this[`q1${i}`] = document.getElementById(`q1-${i}`);
  let q1Dom = this[`q1${i}`];
  q1Dom.innerHTML = q1;

  this[`q2${i}`] = document.getElementById(`q2-${i}`);
  let q2Dom = this[`q2${i}`];
  q2Dom.innerHTML = q2;

  this[`q3${i}`] = document.getElementById(`q3-${i}`);
  let q3Dom = this[`q3${i}`];
  q3Dom.innerHTML = q3;

  this[`q4${i}`] = document.getElementById(`q4-${i}`);
  let q4Dom = this[`q4${i}`];
  q4Dom.innerHTML = q4;

  this[`phi${i}`] = document.getElementById(`phi-${i}`);
  let phiDom = this[`phi${i}`];
  phiDom.innerHTML = phi;
